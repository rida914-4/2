...
class FuseCustom(Passthrough):
	...
	def getattr(self, path, fh=None):
		...
		#logging.debug("Get the attributes of the file system.\n")
		full_path = self._full_path(path)
		#logging.debug("THIS IS THE PATH {}".format(full_path))
		...
		logging.debug('The attribute dictionary is as follows.\n{}'.format(attr_dict))
    	...
    	def readdir(self, path, fh):
	    	""" This mandatory function is required to read the fuse 
	    		mounted directory	    	
	    	 """
	    	logging.debug("Reading the mounted directory.\n")
		dirents = ['.', '..']
    	...
    	def open(self, path, flags):
        	logging.debug("open the directory.\n")
        	full_path = self._full_path(path)
        	return os.open(full_path, flags)

    	def create(self, path, mode, fi=None):
        	logging.debug("Create the mounted directory.\n")
        	full_path = self._full_path(path)
	...
	def process_file_path(self, path):
		...
		logging.debug("PROCESS - Number : {}".format(number))
		...
		logging.debug("PROCESS - Block ID = {}".format(block_id))
		logging.debug("PROCESS - Inode position in the block = {}".format(inode_position))
		logging.debug("PROCESS - Inode size in the block = {}".format(inode_size))
		
		offset = int(inode_position) * self.INODE_SIZE
		logging.debug("PROCESS - Offset = Number {} x INODE SIZE {} =  : {}".format(inode_position, self.INODE_SIZE, offset))
	...
	def read(self, path, length, offset, fh):
		...
		logging.debug("Read")
		block_id, inode_position, offset = self.process_file_path(path)
		self.get_inode_filename_pair(block_id, inode_position, offset, path)
...
#		no_of_blocks = no_of_blocks - 1
		...
		for block_id, inode_block in self.db_dict.iteritems():
			...
			if int(block_id) <= self.BLOCK_WRITEABLE:
				...
				for offset in range(0, self.INODE_SIZE * self.INODE_PER_BLOCK, self.INODE_SIZE):
					...
					logging.debug("Block ID : {}, index : {} ".format(block_id, offset))
					...
					if file_number_bin != (b'', 0):
						...
						logging.debug("Block ID : {}, index : {}, file number : {}, ".format(block_id, offset, file_number))
						file_list.append(file_number)
					else:
						pass
						logging.debug('Empty byte {}'.format(file_number_bin))
	...
	def get_inode_file_list(self, block_id, inode_position, offset):
		...
		files = list()
		logging.debug("get_inode_file_list")
		...
		for index, a in enumerate(range(offset-40, offset, 4)):
			...
			if index in range(1, 9):
				...
				logging.debug('File number {}, {}, {}, {}'.format(file_number, a, a+4, raw_content))
	...
	def get_inode_filename_pair(self, block_id, inode_position, offset, path):
		''' Pull the data from the db return dictionary '''
		...
		for index1, block in enumerate(file_data):
			
			# We will deal with each 8 x 128 bytes seperately

			for index, a in enumerate(range(offset-8, 1024, 8)):

				# Dissect the block, every 128 byte is a pair
				logging.debug('Block : {}, offset start: {}, offset end : {} '.format(block, offset-8, offset))
				
				raw_content = ((block, 0)[0][a:a+8], 0)
				
				# Divide into 64 nad 64 for inode and file name pair
				
				inode_raw = (raw_content[0][0:3], 0)
				filename = (raw_content[0][4:8], 0)

				inode_num = self.hex_to_int(inode_raw[0][0])
				file_number = self.hex_to_int(filename[0][0])
				
				# Test : Check if the file numbers and the offsets are correct
				logging.debug('File number {}, {}, {}, {}'.format(file_number, a, a+4, raw_content))
				files.append((inode_num, file_number))
				
				offset += 8

		logging.debug('Inode file and name found : {}'.format([a for a in files if inode_position == a[0]]))
		
		return files, [a for a in files if inode_position == a[0]]
	...
	def create_inode_from_leveldb(self):
		...
		for block_id in range(0, self.BLOCK_WRITEABLE):
			...
			for inode_no in range(old_file_no, old_file_no + self.INODE_PER_BLOCK):	
				...
				logging.debug('{},{},{}'.format(self.INODE_PER_BLOCK * self.BLOCK_WRITEABLE, old_file_no, old_file_no + self.INODE_PER_BLOCK))
				...
				inode += inode_bytes
				logging.debug("Inode = {}".format(len(inode_bytes)))
	#			logging.debug("Putting {} as inode no. {}".format((inode_bytes, 0), self.hex_to_int(inode_bytes)))			
				...
				logging.debug("Counter {}, last bound {}".format(counter, indirect_data_bound))
				
				for data_block in range(counter, indirect_data_bound):
				
					# special inode list saves all file names
					if not (inode_no, counter) in special_inode_list:
						special_inode_list.append((inode_no, counter))
					
					indirect_bytes =  ('%%0%dx' % (byte_length << 1) % data_block).decode('hex')[-byte_length:]
	#				logging.debug("Putting 8x 4B data ({}) {} as  {}".format(data_block, (indirect_bytes, 0), self.hex_to_int(indirect_bytes)))	
		
					indirect_data += indirect_bytes
			
				counter += 8
			
				logging.debug(len(indirect_data))			
				# !Test : Check if right number of inodes and data blocks are created : PASSED
				logging.debug("Block : {}, Inode : {}, Data block starts from {} and ends at {} , Bytes: \n".format(block_id, inode_no, counter_old, counter))
				inode += indirect_data
				logging.debug("Inode = {}".format(len(inode)))
			
				# Put extra 4 bytes here
				for a in range(0, 4):
					inode += empty_bytes
				logging.debug("Put empty bytes {} as {}".format((empty_bytes, 0), self.hex_to_int(empty_bytes)))
				...
				logging.debug("Final Block ID({}) single inode : ({})".format(block_id, len(bytes(full_block))))
				logging.debug("Final Block ({}) : {} , {}:".format(len(bytes(full_block)), (full_block, 0), self.hex_to_int(bytes(full_block))))
			
				old_file_no = inode_no + 1
			
			logging.debug("Final Block ID({}) all inode : ({}), last seen inode {} ".format(block_id, len(bytes(full_block)), inode_no))
		...
		logging.debug("CREATING DATABASE - Total blocks = {}".format(total_blocks))
		...
		logging.debug(len(data_bytes), data_bytes)
		...
		for index, pair in enumerate(special_inode_list):
			...
			if pair and len(pair) == 2: 
				...
				logging.debug("Inode converted into 128 byte binary array {} : {}".format(len(special_bytes), self.hex_to_int(special_bytes)))
				
				special_bytes += ('%%0%dx' % (byte_length << 1 ) % filename).decode('hex')[-byte_length:]
				logging.debug("Inode converted into 128 byte binary array {} : {}".format(len(special_bytes), self.hex_to_int(special_bytes)))
				
				logging.debug('{}'.format(special_bytes, 0))
				
				multiples_of_128 = range(0, (8+1)*128, 128)[1:]

				if index in multiples_of_128:
					multiples_of_128.remove(index)
		...
		for indirect_pointers in range(self.BLOCK_WRITEABLE, total_blocks):
			...
			if indirect_pointers in range(self.BLOCK_WRITEABLE , self.BLOCK_WRITEABLE + 8):

				logging.debug('SPECIAL Data block pointer/address :{}, Range : {} to {}'.format(indirect_pointers, count, count + 8))
				logging.debug('{}'.format(special_list[count:count+8]))
				logging.debug('{},{},{}'.format(indirect_pointers, count, len(special_list[count])))
				db.put(b'{}'.format(indirect_pointers), b'{}'.format(special_list[count]))
				count += 1
		...
		db.close()
		logging.debug('Files created : {}'.format(files_created))
#		logging.debug len(temp['41'])
