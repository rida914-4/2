...
class FuseCustom(Passthrough):
	...
	def getattr(self, path, fh=None):
		...
		logging.debug("Final directory entries: {}".format(dirents))
	...
	def write(self, path, buf, offset, fh):
		logging.debug("Write operation called.pth:{} buf:{} offset:{} fh:{}\n".format(path, buf, offset, fh))
		#put_key_value_into_level_db(path.split('/')[-1], buf)
        	return os.write(fh, buf)
   
    	def read(self, path, length, offset, fh):
    
        	logging.debug("FUSE - Read operation is called for file {} .".format(path))
        	try:
